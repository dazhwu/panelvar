...
void
update_commandStr(model &m) {
    m.command_str = "";
    for (gmm_var var : m.Dgmm_vars) {
        m.command_str +=
            (" gmm(" + var.name + "," + std::to_string(var.min_lag) + ":" + std::to_string(var.max_lag) + ") ");
    }
...
void
list_to_exog_iv(int T, vector<model> &list_models, List_Variables &the_list, int dest) {
...
void
list_to_Dep(int T, vector<model> &list_models) {
...
void
add_Dep(model &m, int first_lag, int last_lag) {
    int num_var = Endo_list.names.size();
...
void
list_to_gmm(int T, vector<model> &list_models, List_Variables &the_list) {
    int num_var = the_list.names.size();
    for (int i = 0; i < num_var; ++i) {
...
void
add_gmm(model &m, string var_name, int min_lag, int max_lag) {
    gmm_var new_var = gmm_var(var_name, min_lag, max_lag, 0);
...
void
explode_gmm(int min_lag, int T, vector<model> &list_models, string var_name) {
    vector<model> new_models;
    for (std::size_t m = 0, max = list_models.size(); m < max; ++m) {
...
void
explode_exog_iv(int max_lag, int T, vector<model> &list_models, string var_name, int dest) {
    vector<model> new_models;
    for (std::size_t m = 0, max = list_models.size(); m < max; ++m) {
        for (int j = max_lag + 1; j <= T - 1; ++j) {
            model new_m = list_models[m];
