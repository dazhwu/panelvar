...
std::vector<string> combined_vector(std::vector<string> v1,
...
void lag(Ref <RowMatrixXd> mat, Ref <RowMatrixXd> lagged, int N, int lag_number,
         double fill) {
		...
		for (int i = 0; i < N; ++i) {
			...
			for (int j = height - 2; j >= 0; --j) {
				if (isnan(ori_i(j + 1, 0)) || (is_na[j + 1 + (i * height)] == 1)) {
					...
					tbr_i(j + 1, 0) = (ori_i(j, 0) - this_sum * (1.0 / this_count)) *
