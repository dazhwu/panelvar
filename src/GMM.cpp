...
returned_result::returned_result(struct Regression &reg, Hansen_test_info &_hansen, VectorXcd &_stability,
...
special_process(Ref<RowMatrixXd> z_list, Ref<RowMatrixXd> Cx_list, Ref<RowMatrixXd> Cy_list, temp_result &tmp_res,
...
regular_process(Eigen::Ref<RowMatrixXd> z_list, std::unordered_map<string, RowMatrixXd> &final_xy_tables,
                struct basic_info &model_info, struct model_options &options) {
    ...
    std::tie(xz_list, zy_list, zHz_list) =
        calculate_basic(z_list, Cx_list, Cy_list, H1, N, z_height, x_height, x_width, y_width);
    ...
    Hansen_test_info hansen = hansen_overid(results[1].W_inv, results[1].zs, model_info.num_instr * model_info.num_dep,
                                            model_info.num_indep * model_info.num_dep, N);
    // 考虑把W_inv计算加到W_next后边去   //！！！！查在onestep条件下，是否用result2.zs, 关系到是否把 ZuuZ计算单列

    // 查在onestep条件下，是否用result2.beta
    VectorXcd stability = stability_test(results[1].beta, model_info.num_dep_lags);

    Regression tbr =
        Regression(results[num_steps - 1].beta, results[num_steps - 1].vcov, results[num_steps - 1].std_err,
                   results[num_steps - 1].W, z_list, Cy_list, Cx_list, results[num_steps - 1].residual);   //_residual_t
    ...
    ////cout << "MMSC_LU " <<endl;
    returned_result the_result = returned_result(tbr, hansen, stability, model_info, options, "");
    return std::make_tuple(the_result, tmp_res);
}

void
core_GMM(temp_result &tmp_res, Ref<RowMatrixXd> Cx_list, Ref<RowMatrixXd> Cy_list, Ref<RowMatrixXd> z_list,
         MatrixXd &_z_t_list, int N, int T, int num_obs, int x_width, int y_width, int z_height, int diff_width,
         string transformation, bool level) {

    RowMatrixXd _XZ, _Zy, _XZ_t, _Zy_t, W1;
    std::tie(_XZ, _Zy, _XZ_t, _Zy_t, W1) =
        calculate_XZ_ZY_W1(tmp_res.xz_list, tmp_res.zy_list, tmp_res.zHz_list, N, x_width, y_width, z_height);

    ////cout <<"----GMM" << endl;
    Step_Result result1 = GMM_step(N, num_obs, z_list, _z_t_list, Cx_list, Cy_list, _XZ, _XZ_t, _Zy, _Zy_t, W1);

    Step_Result result2 =
        GMM_step(N, num_obs, z_list, _z_t_list, Cx_list, Cy_list, _XZ, _XZ_t, _Zy, _Zy_t, result1.W_next);
...
void
update_MMSC_LU(struct basic_info &model_info, Hansen_test_info &hansen) {
...
Step_Result
GMM_step(int N, int num_obs, Ref<RowMatrixXd> z_list, MatrixXd &z_list_t, Ref<RowMatrixXd> Cx_list,
         Ref<RowMatrixXd> Cy_list, RowMatrixXd &_XZ, RowMatrixXd &_XZ_t, RowMatrixXd &_Zy, RowMatrixXd &_Zy_t,
         RowMatrixXd &W) {
    ...
    RowMatrixXd beta = (_M_XZ_W * _Zy_t).reshaped<Eigen::RowMajor>(Cx_list.cols(), Cy_list.cols());
    ...
    */
    Step_Result current_step =
        Step_Result(W, W_inv, W_next, _XZ_W, M, _M_XZ_W, zs, qs, ZuuZ, _zs_list, beta, residual, _residual_t);
    return (current_step);
}

std::tuple<RowMatrixXd, RowMatrixXd, RowMatrixXd, RowMatrixXd>
calculate_ZuuZ(int N, Ref<RowMatrixXd> z_list, RowMatrixXd &residual, RowMatrixXd &diag_dep, int y_width) {
...
#pragma omp parallel for reduction(+ : zs, qs, ZuuZ)
    for (int i = 0; i < N; ++i) {
        Map<RowMatrixXd> u(residual.middleRows(i * r_height, r_height).data(), r_height * y_width, 1);

        //    if(i==0)
        //      //cout <<z_list.middleRows(i * z_height, z_height) <<endl;

        qs.noalias() += z_list.middleRows(i * z_height, z_height) * residual.middleRows(i * r_height, r_height);
...
calculate_XZ_ZY_W1(RowMatrixXd &xz_list, RowMatrixXd &zy_list, RowMatrixXd &zHz_list, int N, int x_width, int y_width,
                   int z_height) {
...
std::tuple<RowMatrixXd, RowMatrixXd, RowMatrixXd>
calculate_basic(Ref<RowMatrixXd> z_list, Ref<RowMatrixXd> Cx_list, Ref<RowMatrixXd> Cy_list, RowMatrixXd &H, int N,
                int z_height, int x_height, int x_width, int y_width) {
...
RowMatrixXd
calculate_residual(Ref<RowMatrixXd> y_list, Ref<RowMatrixXd> x_list, Ref<RowMatrixXd> beta) {
...
RowMatrixXd
vcov_step_1(Ref<RowMatrixXd> _M_XZ_W, Ref<RowMatrixXd> W2, int N) {
...
RowMatrixXd
vcov(Ref<RowMatrixXd> z_list, Ref<RowMatrixXd> Cx, Ref<RowMatrixXd> M2, Ref<RowMatrixXd> _M2_XZ_W2,
     Ref<RowMatrixXd> _W2_inv, Ref<RowMatrixXd> zs2, int step, int N) {
    RowMatrixXd tbr;
    ...
    tbr = Windmeijer(M2, _M2_XZ_W2, _W2_inv, zs2, vcov_step_previous, Cx, z_list, residual1, N);
    return tbr;
}
...
*/
RowMatrixXd
get_H1(int width, int diff_width, int T, string transformation, bool level) {
    ...
    if (transformation == "fd") {
        ...
        if (width > diff_width) {
            for (int i = diff_width; i < width; ++i)
                tbr(i, i) = 1;
            Eigen::Ref<RowMatrixXd> low_left = tbr.block(diff_width, 0, width - diff_width, diff_width);
            for (int i = 0; i < diff_width; ++i) {
                low_left(i, i) = -1;
                low_left(i + 1, i) = 1;
            }
            Eigen::Ref<RowMatrixXd> up_right = tbr.block(0, diff_width, diff_width, width - diff_width);
            for (int i = 0; i < diff_width; ++i) {
                up_right(i, i) = -1;
                up_right(i, i + 1) = 1;
            }
...
RowMatrixXd
get_H1_fod(int width, int diff_width, int T, bool level) {
...
RowMatrixXd
generate_D_matrix(int height, int T) {
    //	//cout<<height<<endl;
...
RowMatrixXd
Windmeijer(Ref<RowMatrixXd> M2, Ref<RowMatrixXd> _M2_XZ_W2, Ref<RowMatrixXd> W2_inv, Ref<RowMatrixXd> zs2,
           Ref<RowMatrixXd> vcov_step1, Ref<RowMatrixXd> Cx_list, Ref<RowMatrixXd> z_list, Ref<RowMatrixXd> residual1,
           int N) {
    ...
    RowMatrixXd diag_dep = RowMatrixXd::Identity(y_width, y_width);
    RowMatrixXd D = RowMatrixXd::Zero(z_height * y_width * y_width * x_width, z_height * y_width);
    int z_width = z_list.cols();
    ...
    for (int i = 0; i < N; ++i) {
        ...
        zx = Eigen::kroneckerProduct(temp_xz, diag_dep);   // Eigen::kroneckerProduct(z*x, diag_dep);
