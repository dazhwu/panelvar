...
get_z_table(int N, int T, struct df_info info, Ref<RowMatrixXd> Dgmm_dat,
            ...
            vector<gmm_var> Lgmm_vars, vector<regular_variable> iv_vars,
            bool level, string transformation, bool collapse) {
  ...
  */
  ...
  z_table =
      z_table.unaryExpr([](double v) { return std::isfinite(v) ? v : 0.0; });
...
#pragma omp parallel for
  for (int i = 0; i < N; ++i) {
    ...
    Ref<RowMatrixXd> array_Lgmm =
        Lgmm_dat.block(i * Lgmm_iv_height, 0, Lgmm_iv_height, Lgmm_width);
    Ref<RowMatrixXd> array_iv =
        iv_dat.block(i * Lgmm_iv_height, 0, Lgmm_iv_height, iv_width);
    for (std::size_t var_id = 0, max = Lgmm_vars.size(); var_id != max;
         ++var_id) {
...
int prepare_z_gmm_level(vector<gmm_var> Lgmm_vars, struct df_info info,
                        int level_width, bool collapse) {
...
struct z_info calculate_z_dimension(vector<gmm_var> Dgmm_vars,
                                    ...
                                    struct df_info info, bool level,
  ...
  if (level) {
    int num_Lgmm_instr =
        prepare_z_gmm_level(Lgmm_vars, info, level_width, collapse);
	if(options.constant)
...
void build_z_diff(int N, vector<gmm_var> Dgmm_vars,
                  vector<regular_variable> iv_vars, struct df_info info,
                  struct z_info z_information, Ref<RowMatrixXd> Dgmm_dat,
                  Ref<RowMatrixXd> Div_dat, bool level, string transformation,
                  bool collapase) {
...
#pragma omp parallel for
  for (int i = 0; i < N; ++i) {
    Ref<RowMatrixXd> z = z_table.block(i * z_height, start_col, z_height,
                                       diff_width - start_col);
    ...
    for (std::size_t var_id = 0, max = Dgmm_vars.size(); var_id != max;
         ++var_id) {
    ...
    for (std::size_t var_id = 0, max = iv_vars.size(); var_id != max;
         ++var_id) {
...
void prepare_Z_iv_diff(vector<regular_variable> iv_vars, int width,
                       struct df_info info) {
  ...
  int j;
  for (j = 0; j < width; ++j) {
    Eigen::VectorXi tempCol =
        Eigen::VectorXi::Constant(num_iv, 1, info.first_diff_index + j);
