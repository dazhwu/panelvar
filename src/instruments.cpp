...
get_z_table(int N, int T, struct df_info info, Ref<RowMatrixXd> Dgmm_dat, Ref<RowMatrixXd> Lgmm_dat,
            Ref<RowMatrixXd> iv_dat, Ref<RowMatrixXd> Div_dat, vector<gmm_var> Dgmm_vars, vector<gmm_var> Lgmm_vars,
            vector<regular_variable> iv_vars, bool level, string transformation, bool collapse) {
    struct z_info z_information =
        calculate_z_dimension(Dgmm_vars, Lgmm_vars, iv_vars, info, level, transformation, collapse);
    /*
    ...
    */
    if (level)
        build_z_level(N, Dgmm_vars, Lgmm_vars, iv_vars, info, z_information, Dgmm_dat, Lgmm_dat, Div_dat, iv_dat, level,
                      transformation, collapse);
    else
        build_z_diff(N, Dgmm_vars, iv_vars, info, z_information, Dgmm_dat, Div_dat, level, transformation, collapse);
...
void
build_z_level(int N, vector<gmm_var> Dgmm_vars, vector<gmm_var> Lgmm_vars, vector<regular_variable> iv_vars,
              struct df_info info, struct z_info z_information, Ref<RowMatrixXd> Dgmm_dat, Ref<RowMatrixXd> Lgmm_dat,
              Ref<RowMatrixXd> Div_dat, Ref<RowMatrixXd> iv_dat, bool level, string transformation, bool collapse) {
...
// Eigen::MatrixXi temp_col=Eigen::MatrixXi::Constant(1,1,
// z_information.z_width-start_col);
#pragma omp parallel for
    for (int i = 0; i < N; ++i) {
        Ref<RowMatrixXd> z =
            z_table.block(i * z_information.z_height, 0, z_information.z_height, z_information.z_width);
        ...
        for (std::size_t var_id = 0, max = Lgmm_vars.size(); var_id != max; ++var_id) {
            ...
            Ref<MatrixXi> t_info = gmm_level_info.block(3 * var_id, 0, 3, level_info_width);
        ...
        for (int var_id = 0; var_id < len_iv_vars; ++var_id) {
            for (int j = 0; j < z_information.level_width; ++j) {
                z(start_pos + var_id, j + z_information.diff_width) = array_iv(info.first_level_index + j, var_id);
            }
            // std:://cout << the_row << " " << start_col+j << array_Lgmm(the_index,
            // var_id) << std::endl;
        }
    }
}
...
int
prepare_z_gmm_level(vector<gmm_var> Lgmm_vars, struct df_info info, int level_width, bool collapse) {
...
struct z_info
calculate_z_dimension(vector<gmm_var> Dgmm_vars, vector<gmm_var> Lgmm_vars, vector<regular_variable> iv_vars,
                      struct df_info info, bool level, string transformation, bool collapse) {
    ...
    if (level) {
        ...
        if (options.constant)
            ...
            level_height = num_Lgmm_instr;
    }
    int num_Dgmm_instr = prepare_Z_gmm_diff(Dgmm_vars, info, level, transformation, collapse);
    ...
    z_info z_information =
        z_info(diff_height, diff_width, level_width, level_height, z_height, z_width, num_Dgmm_instr, num_Lgmm_instr);
    return z_information;   //   , gmm_diff_info, iv_diff_info, gmm_level_info)
}

void
build_z_diff(int N, vector<gmm_var> Dgmm_vars, vector<regular_variable> iv_vars, struct df_info info,
             struct z_info z_information, Ref<RowMatrixXd> Dgmm_dat, Ref<RowMatrixXd> Div_dat, bool level,
             string transformation, bool collapase) {
...
#pragma omp parallel for
    for (int i = 0; i < N; ++i) {
        ...
        // z_width = z.shape[1]
        Ref<RowMatrixXd> array_gmm = Dgmm_dat.block(i * gmm_Div_height, 0, gmm_Div_height, gmm_width);

        Ref<RowMatrixXd> array_fd_iv = Div_dat.block(i * gmm_Div_height, 0, gmm_Div_height, div_width);
...
void
prepare_Z_iv_diff(vector<regular_variable> iv_vars, int width, struct df_info info) {
...
int
prepare_Z_gmm_diff(vector<gmm_var> Dgmm_vars, struct df_info info, bool level, string transformation, bool collapse) {
    ...
    for (int var_id = 0; var_id < num_gmm; ++var_id) {
        ...
        for (int j = 0; j < width; ++j) {
            ...
            int tstart = tend + Dgmm_vars[var_id].min_lag - Dgmm_vars[var_id].max_lag;
            if (tstart < 0)
                tstart = 0;
