...
struct df_info get_info(model_options options, model m) {
	...
	if ((options.level) && (options.transformation == "fod"))
		...
		first_diff_index =
				first_level_index + 1; //# max(max_lag + 1, max_Dgmm_minlag)

	if (first_diff_index + 2 >
	    last_diff_index) // # to do: change 3 to something rated to AR(p)
		throw std::invalid_argument("Not enough periods to run the model");
	struct df_info tbr = df_info(N, T, first_diff_index, first_level_index,
	                             last_diff_index, last_level_index, max_lag);
	return tbr;
}
...
vector<returned_result> prepare_data(vector<string> &identifiers, Ref <RowMatrixXd> pdata,
	...
	for (std::size_t m = 0, max = the_models.size(); m < max; ++m) {
		...
		std::tie(z_table, z_information) = get_z_table(
				N, T, info, Dgmm_table, Lgmm_table, iv_table, Delta_iv_table,
				the_models[m].Dgmm_vars, the_models[m].Lgmm_vars, the_models[m].iv_vars,
		...
		temp_result tmp_res;
		try {
			std::tie(model_result, tmp_res) =
					regular_process(z_table, final_xy_tables, model_info, options);
			...
			vector<RowMatrixXd> upper, lower;
			std::tie(lower, upper)=Bootstrapping(options.irf, num_draws, ahead, z_table, na_records, tmp_res, model_info, options);
...
void process_residual(returned_result &m, VectorXi &na_records){
...
struct basic_info generate_model_info(int N, int T, Ref <RowMatrixXd> z_table, VectorXi &na_records,
	...
	std::tie(num_obs, max_obs, min_obs, avg_obs) =
			prepare_reg(z_table, final_xy_tables["Cx"], final_xy_tables["Cy"], na_records,
			            z_information, options.transformation, options.level);
...
void handle_tables(Ref <RowMatrixXd> pdata, Ref <RowMatrixXd> fd_data, model &m,
                   vector<string> &df_cols, df_info info) {
	get_gmm_tables(pdata, fd_data, m.Dgmm_vars, m.iv_vars, m.Lgmm_vars, df_cols,
	               options.level);
...
void get_gmm_tables(Ref <RowMatrixXd> pdata, Ref <RowMatrixXd> fd_data,
                    vector<gmm_var> &Dgmm_vars,
                    vector<regular_variable> &iv_vars,
                    vector<gmm_var> &Lgmm_vars, vector<string> &df_cols,
                    bool level) {
...
RowMatrixXd gen_table(Ref <RowMatrixXd> ori_data, vector<ty> variable_list,
	...
	// variable_names = vairable_list.names; //   [var.name for var in
	// variable_list]
...
#pragma omp parallel for
			for (int i = 0; i < N; ++i) {
				...
				// if(i==0)
				//   //cout<<tbr_i(height-1,0)<<endl;
				//tbr_i.block(lag, 0, height - lag, 1) =  ori_i.block(0, 0, height - lag, 1)*row_has_nan(ori_i.block(lag, 0, height - lag, 1));
...
void get_xy_table_dict(Ref <RowMatrixXd> pdata,
                       model &m,
                       vector<string> &df_cols, string transformation) {
	...
	/*	RowMatrixXd gen_table(Ref <RowMatrixXd> ori_data, vector <ty>
	   variable_list, vector <string> df_cols)
	*/
	...
	if (transformation == "fd") {
		...
		//saveData("ori_x.csv", xy_tables["x"]);
		//saveData("ori_y.csv", xy_tables["y"]);

		// //saveData("Dx.csv", xy_tables["Dx"]);
...
void get_final_xy_tables(df_info info, bool level, string transformation) {
	...
	if (level) {
		std::tie(Cy, Cx) = get_final_xy_systemGMM(
				xy_tables["Dx"], xy_tables["Dy"], xy_tables["x"], xy_tables["y"], cut,
				Dcut, cut_height, Dcut_height, transformation);
	} else {
		std::tie(Cy, Cx) = get_final_xy_diffGMM(xy_tables["Dy"], xy_tables["Dx"],
		                                        Dcut, Dcut_height);
	}
...
get_final_xy_systemGMM(Ref <RowMatrixXd> Dx, Ref <RowMatrixXd> Dy,
                       Ref <RowMatrixXd> x, Ref <RowMatrixXd> y, vector<int> cut,
                       vector<int> Dcut, int cut_height, int Dcut_height,
...
#pragma omp parallel for
	for (int i = 0; i < N; ++i) {
		...
		if (transformation == "fod") {
			...
			//temp_x.row(0) = RowMatrixXd::Constant(1, temp_x.cols(),NAN);                  //Zero(1, temp_x.cols());
		}
...
std::tuple<RowMatrixXd, RowMatrixXd> get_final_xy_diffGMM(Ref <RowMatrixXd> Dy,
                                                          Ref <RowMatrixXd> Dx,
                                                          vector<int> Dcut,
                                                          int Dcut_height) {
	int Dx_height = Dx.rows() / N;
...
prepare_reg(Ref <RowMatrixXd> z_list, Ref <RowMatrixXd> Cx, Ref <RowMatrixXd> Cy,
            VectorXi &na_records, z_info z_information, string transformation, bool level) {
...
std::tuple<vector<RowMatrixXd>,vector<RowMatrixXd> > Bootstrapping(string method, int num_draws, int ahead, Ref<RowMatrixXd> z_table,
	...
	vector<vector<RowMatrixXd>> All_Mats(num_draws);
...
//#pragma omp parallel for
	for (int i=0; i< num_draws; ++i){
		...
		for (int j=0; j<N; ++j){
			...
			pseudo_Cx_list.middleRows(j*model_info.x_height, model_info.x_height)=Cx_list.middleRows(the_id*model_info.x_height, model_info.x_height);
			pseudo_Cy_list.middleRows(j*model_info.x_height, model_info.x_height)=Cy_list.middleRows(the_id*model_info.x_height, model_info.x_height);
			pseudo_z_list.middleRows(j*model_info.num_instr, model_info.num_instr)=z_table.middleRows(the_id*model_info.num_instr, model_info.num_instr);
			pseudo_na_records.middleRows(j*model_info.x_height, model_info.x_height)=na_records.middleRows(the_id*model_info.x_height, model_info.x_height);

			pseudo_xz_list.middleRows(j*model_info.num_instr, model_info.num_instr)=tmp_res.xz_list.middleRows(the_id*model_info.num_instr, model_info.num_instr);
			pseudo_zy_list.middleRows(j*model_info.num_instr, model_info.num_instr)=tmp_res.zy_list.middleRows(the_id*model_info.num_instr, model_info.num_instr);
			pseudo_zHz_list.middleRows(j*model_info.num_instr, model_info.num_instr)=tmp_res.zHz_list.middleRows(the_id*model_info.num_instr, model_info.num_instr);

		}
		temp_result pseudo_t_res=temp_result(tmp_res.H, pseudo_xz_list, pseudo_zy_list, pseudo_zHz_list);
		RowMatrixXd residual, beta;
		std::tie(beta, residual)=special_process(pseudo_z_list, pseudo_Cx_list, pseudo_Cy_list, pseudo_t_res, model_info, options);
		All_Mats[i]=irf(method, residual, pseudo_na_records,
	                                     beta, ahead, model_info.num_dep_lags);
	...
	choose_U_L(All_Mats, upper, lower, num_draws, model_info.num_dep, ahead);
...
void choose_U_L(vector<vector<RowMatrixXd>> &All_Mats, vector<RowMatrixXd> &upper, vector<RowMatrixXd> &lower, int num_draws, int num_dep, int ahead ){
	...
	//temp.reserve(num_draws);
	for (int m=0; m<num_dep; ++m){
		for (int i=0; i<ahead;++i){  //row
			for (int j=0; j<num_dep;++j){ //col
				vector<double> temp(num_draws);
				//cout << temp.size()<<endl;
#pragma omp parallel for

				for (int k=0; k<num_draws; ++k) {
					Ref <RowMatrixXd> the_mat = (All_Mats[k])[m];

					temp[k] = the_mat(i, j);
				}
				//std::sort(temp.begin(), temp.end());
				std::nth_element(temp.begin(), temp.begin() + L, temp.end());
				Ref <RowMatrixXd> lower_mat =lower[m];
				lower_mat(i,j)=temp[L];


				std::nth_element(temp.begin(), temp.begin() + U, temp.end() );
				Ref <RowMatrixXd> upper_mat =upper[m];

				upper_mat(i,j)=temp[U];
			}
		}
