...
process_command(int T_MAX, string dep, int lags, string exog, string gmm, string options_str,
                vector<string> &df_col_names) {
...
// df.columns)
Command::Command(int T_MAX, string dep, int lags, string exog, string gmm, string options_str,
                 vector<string> &df_col_names) {
...
void
Command::parse_dep(string dep, int lags) {
...
bool
Command::parse_spaced_vars_range(string var, List_Variables &dest_list) {
...
bool
Command::parse_spaced_vars_single(string var, List_Variables &dest_list) {
...
bool
Command::parse_spaced_vars_auto(string var, List_Variables &dest_list) {
...
// }
void
Command::parse_spaced_vars(vector<string> list_vars, List_Variables &dest_list) {
    bool isMatch;
    for (string var : list_vars) {
        ...
        if (!isMatch) {
            ...
            if (!isMatch)
                isMatch = parse_spaced_vars_auto(var, dest_list);   // e.g., L(1:?).variable
...
void
Command::parse_gmmStyle(vector<string> &matching_parts, string gmm) {
    smatch match;
    string temp = gmm;
...
void
Command::process_GMM(vector<string> &vars, int min_lag, int max_lag, string part, bool auto_mode) {
    if (min_lag > max_lag)
    ...
    for (string var : vars) {
        ...
        if (variable_exists(var, DGMM_list.names))
            throw std::invalid_argument(part + ": " + var + " cannot be declared as GMM variable for twice or more");
...
void
Command::parse_IV(vector<string> &matching_parts, string gmm) {
    smatch match;
    string temp = gmm;
    regex r("iv[(].{1,}[)]");
    while (regex_search(temp, match, r)) {
...
void
Command::parse_options(string options_str) {
...
Command::check_dep() {
    ...
    for (std::size_t i = 0; i != Endo_list.names.size(); ++i) {
        ...
        if (list_dep_lags.size() == 0)
            throw std::invalid_argument("lagged dependent variable should be included");
    }
}
...
void
Command::check_three_lists() {
    for (auto iv_name : IV_list.names) {
        if (variable_exists(iv_name, Endo_list.names))
            throw std::invalid_argument("variable " + iv_name + ": a dependent variable cannot be instrument variable");
