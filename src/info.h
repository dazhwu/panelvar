...
struct Regression {
    ...
    Regression();
    Regression(RowMatrixXd &_beta, RowMatrixXd &_vcov, RowMatrixXd &_std_err, RowMatrixXd &w, Ref<RowMatrixXd> _z,
               Ref<RowMatrixXd> _Cy, Ref<RowMatrixXd> _Cx, RowMatrixXd &_residual);
};
...
struct basic_info {
    ...
    basic_info(int _N, int _T, int _num_obs, int _num_instr, int _num_dep, int _num_dep_lags, int _num_indep,
               int _diff_width, int _x_height, int _max_obs, int _min_obs, double _avg_obs,
               vector<regular_variable> &dep_indep, vector<string> &identifiers, model_options &options);
};
